...
public class TerrainHeightMap : MonoBehaviour
{
    ...
    private void ProcessHeightMap()
    {
        while (lineIndex < terrainText.Length)
        {
            ...
            if (currentLine.StartsWith("f"))
            {
                ...
                {
                    Debug.Log(
                        "Vertices couldnt be parsed, Check format or cultureinfo" + e.Message
                    );
                    break;
                }
                ...
                for (int f = startX; f <= endX; f++)
                {
                    for (int d = startZ; d <= endZ; d++)
                    {
                        ...
                        if (IsPointInsideTriangle(p1, p2, p3, new Vector2(X, Z)))
                        {
                            ...
                            if (
                                GetHeight(vertex1, vertex2, vertex3, new Vector2(X, Z))
                                > heightMap[f, d]
                            )
                                heightMap[f, d] = GetHeight(
                                    vertex1,
                                    vertex2,
                                    vertex3,
                                    new Vector2(X, Z)
    ...
    public Vector3 GetPosition(Vector3 position)
    {
        try
        {
            return new Vector3(
                position.x,
                heightMap[(int)(position.x * (-1)), (int)position.z],
                position.z
            );
        }
        catch (IndexOutOfRangeException)
